...
const LINKING_ERROR = `The package '@lottiefiles/dotlottie-react-native' doesn't seem to be linked. Make sure: \n\n` + Platform.select({
...
const COMMAND_SET_ADD_STATE_MACHINE_EVENT_LISTENER = 'addStateMachineEventListener';
const COMMAND_SET_REMOVE_STATE_MACHINE_EVENT_LISTENER = 'removeStateMachineEventListener';
...
const DotlottieReactNativeView = UIManager.getViewManagerConfig(ComponentName) != null ? requireNativeComponent(ComponentName) : () => {
  throw new Error(LINKING_ERROR);
};
export const DotLottie = /*#__PURE__*/forwardRef(({
  source,
  ...props
}, ref) => {
  ...
  const playWithUIManager = useCallback(() => {
    ...
    if (command) {
      UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, []);
    }
  }, []);
  ...
  const stopWithUIManager = useCallback(() => {
    ...
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, []);
    }
  }, []);
  const setLoopWithUIManager = useCallback(value => {
    const command = NativeViewManager.Commands[COMMAND_SET_LOOP];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [value]);
    }
  }, []);
  const setSpeedWithUIManager = useCallback(value => {
    const command = NativeViewManager.Commands[COMMAND_SET_SPEED];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [value]);
    }
  }, []);
  const setPlayModeWithUIManager = useCallback(mode => {
    const command = NativeViewManager.Commands[COMMAND_SET_PLAY_MODE];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [mode]);
    }
  }, []);
  const setFrameWithUIManager = useCallback(frame => {
    const command = NativeViewManager.Commands[COMMAND_SET_FRAME];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [frame]);
    }
  }, []);
  ...
  const setSegmentWithUIManager = useCallback((start, end) => {
    const command = NativeViewManager.Commands[COMMAND_SET_SEGMENT];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [start, end]);
    }
  }, []);
  const startStateMachineWithUIManager = useCallback(stateMachineId => {
    const command = NativeViewManager.Commands[COMMAND_SET_START_STATE_MACHINE];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [stateMachineId]);
    }
  }, []);
  const stopStateMachineWithUIManager = useCallback(() => {
    const command = NativeViewManager.Commands[COMMAND_SET_STOP_STATE_MACHINE];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, []);
    }
  }, []);
  const loadStateMachineWithUIManager = useCallback(stateMachineId => {
    const command = NativeViewManager.Commands[COMMAND_SET_LOAD_STATE_MACHINE];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [stateMachineId]);
    }
  }, []);
  const postEventWithUIManager = useCallback((x, y) => {
    const command = NativeViewManager.Commands[COMMAND_SET_POST_EVENT];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [x, y]);
    }
  }, []);
  const addStateMachineEventListenerWithUIManager = useCallback(() => {
    const command = NativeViewManager.Commands[COMMAND_SET_ADD_STATE_MACHINE_EVENT_LISTENER];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, []);
    }
  }, []);
  const removeStateMachineEventListenerWithUIManager = useCallback(() => {
    const command = NativeViewManager.Commands[COMMAND_SET_REMOVE_STATE_MACHINE_EVENT_LISTENER];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, []);
    }
  }, []);
  const resizeWithUIManager = useCallback((width, height) => {
    const command = NativeViewManager.Commands[COMMAND_SET_RESIZE];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [width, height]);
    }
  }, []);
  const setMarkerWithUIManager = useCallback(marker => {
    const command = NativeViewManager.Commands[COMMAND_SET_MARKER];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [marker]);
    }
  }, []);
  const loadThemeWithUIManager = useCallback(themeId => {
    const command = NativeViewManager.Commands[COMMAND_SET_LOAD_THEME];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [themeId]);
    }
  }, []);
  const loadAnimationWithUIManager = useCallback(animationId => {
    const command = NativeViewManager.Commands[COMMAND_SET_LOAD_ANIMATION];
    if (command) {
      return UIManager.dispatchViewManagerCommand(findNodeHandle(nativeRef.current), command, [animationId]);
    }
  }, []);
  ...
  useImperativeHandle(ref, () => ({
    ...
    removeStateMachineEventListener: removeStateMachineEventListenerWithUIManager,
  ...
  return /*#__PURE__*/_jsx(DotlottieReactNativeView, {
    ref: nativeRef,
    loop: false,
    autoplay: false,
    source: parsedSource || '',
